...
import numbers
from rpchol.kernels import MaternKernel, MaternKernel_vec, MaternKernel_mtx, GaussianKernel, GaussianKernel_vec, \
    GaussianKernel_mtx, LaplaceKernel, LaplaceKernel_vec, LaplaceKernel_mtx
...
class KernelMatrix(FunctionMatrix):
    ...
    def kernel_from_input(kernel, bandwidth=1.0, **kwargs):
        if isinstance(kernel, str):
            if kernel == 'gaussian':
                return partial(GaussianKernel, bandwidth=bandwidth), partial(GaussianKernel_vec,
                                                                             bandwidth=bandwidth), partial(
                    GaussianKernel_mtx, bandwidth=bandwidth)
            elif kernel == 'matern':
                return partial(MaternKernel, bandwidth=bandwidth, nu=kwargs["nu"]), partial(MaternKernel_vec,
                                                                                            bandwidth=bandwidth,
                                                                                            nu=kwargs["nu"]), partial(
                    MaternKernel_mtx, bandwidth=bandwidth, nu=kwargs["nu"])
            elif kernel == 'laplace':
                return partial(LaplaceKernel, bandwidth=bandwidth), partial(LaplaceKernel_vec,
                                                                            bandwidth=bandwidth), partial(
                    LaplaceKernel_mtx, bandwidth=bandwidth)
...
class NonsymmetricKernelMatrix(object):
    ...
    def __init__(self, X, Y, kernel="gaussian", bandwidth=1.0, **kwargs):
        ...
        self.kernel, self.kernel_vec, self.kernel_mtx = KernelMatrix.kernel_from_input(kernel, bandwidth=bandwidth,
                                                                                       **kwargs)
